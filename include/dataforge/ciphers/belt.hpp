...
struct belt_qrk : cvt_qrk<ErrorHandlerT>
{
...
{
    return belt_qrk<>{ std::span{ key }, cmt, std::span{ iv }, pt };
}

}

#include "../detail/ciphers/belt.hpp"
#include "../detail/utility/basic_block_cipher.hpp"

namespace dataforge {

template <IntegralBasedQuark<8> FromQrkT, typename ErrorHandlerT>
struct cvt_resolver<FromQrkT, belt_qrk<ErrorHandlerT>>
{
    using type = generic_block_encrypter<shared_algorithm<basic_block_cipher<belt_cipher_type_factory>>, ErrorHandlerT>;
};

template <IntegralBasedQuark<8> DestQrkT, typename ErrorHandlerT>
struct cvt_resolver<belt_qrk<ErrorHandlerT>, DestQrkT>
{
    using type = generic_block_decrypter<shared_algorithm<basic_block_cipher<belt_cipher_type_factory>>, ErrorHandlerT>;
};
