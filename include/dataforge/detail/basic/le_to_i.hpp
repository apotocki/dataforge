...
class le_to_i
    ...
    , public basic_puller<le_to_i<IT, ByteCountV, ErrorHandlerT>, char, IT, 1>
{
...
public:
    ...
    le_to_i(le_qrk<ErrorHandlerT> const& quark, DestT const&)
    ...
    {
        it_t result = data;
        result |= static_cast<it_t>(ival) << (ByteCountV - 1) * 8;
    ...
    {
        if (num) {
            on_error("unfinished le sequence", *this);
        }
    }
...
class le_to_i<IT, 1, ErrorHandlerT> : public generic_pusher<ErrorHandlerT>
{
    using base_t = generic_pusher<ErrorHandlerT>;

public:
    template <typename DestT>
    le_to_i(be_qrk<ErrorHandlerT> const& quark, DestT const&)
        : base_t{ quark }
    {}
...
struct cvt_resolver<le_qrk<FromEHT>, ToQuarkT>
{
    using type = le_to_i<typename ToQuarkT::integral_type, ToQuarkT::int_qrk_t::bits / 8, FromEHT>;
};
