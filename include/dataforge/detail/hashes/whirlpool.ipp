...
inline const uint_least64_t C0[256] =
{
...
inline const uint_least64_t C1[256] =
{
...
inline const uint_least64_t C2[256] =
{
...
inline const uint_least64_t C3[256] =
{
...
inline const uint_least64_t C4[256] =
{
...
inline const uint_least64_t C5[256] =
{
...
inline const uint_least64_t C6[256] =
{
...
inline const uint_least64_t C7[256] =
{
...
inline const uint_least64_t rc[WHIRLPOOL_R] =
{
...
inline whirlpool_impl::whirlpool_impl()
{
    std::fill(hash_state, hash_state + sizeof(hash_state) / sizeof(word_type), 0);
}

inline void whirlpool_impl::reset()
{
    std::fill(hash_state, hash_state + sizeof(hash_state) / sizeof(word_type), 0);
    bit_count = 0;
}

inline void whirlpool_impl::process_block(const void* msg)
{
    word_type block_holder[8];
    
    // map the buffer to a block:
    word_type* block = be_to_T<8, 64>(block_holder, msg, 64);
    
    // compute and apply K^0 to the cipher state:
    word_type K[8];
    word_type state[8];
    for (int i = 0; i < 8; ++i) {
        state[i] = block[i] ^ (K[i] = hash_state[i]);
    }
    
    // iterate over all rounds:
    word_type L[8];
    for (int r = 0; r < WHIRLPOOL_R; ++r)
    {
        // compute K^r from K^{r-1}:
        for (int offs = 0; offs < 8; ++offs) {
            L[offs] =
                C0[(int)(K[(0 + offs) % 8] >> 56)] ^
                C1[(int)(K[(7 + offs) % 8] >> 48) & 0xff] ^
                C2[(int)(K[(6 + offs) % 8] >> 40) & 0xff] ^
                C3[(int)(K[(5 + offs) % 8] >> 32) & 0xff] ^
                C4[(int)(K[(4 + offs) % 8] >> 24) & 0xff] ^
                C5[(int)(K[(3 + offs) % 8] >> 16) & 0xff] ^
                C6[(int)(K[(2 + offs) % 8] >> 8) & 0xff] ^
                C7[(int)(K[(1 + offs) % 8]) & 0xff];
        }
        L[0] ^= rc[r];
        std::copy(L, L + 8, K);
        ...
        for (int offs = 0; offs < 8; ++offs) {
            L[offs] ^=
                C0[(int)(state[(0 + offs) % 8] >> 56)] ^
                C1[(int)(state[(7 + offs) % 8] >> 48) & 0xff] ^
                C2[(int)(state[(6 + offs) % 8] >> 40) & 0xff] ^
                C3[(int)(state[(5 + offs) % 8] >> 32) & 0xff] ^
                C4[(int)(state[(4 + offs) % 8] >> 24) & 0xff] ^
                C5[(int)(state[(3 + offs) % 8] >> 16) & 0xff] ^
                C6[(int)(state[(2 + offs) % 8] >> 8) & 0xff] ^
                C7[(int)(state[(1 + offs) % 8]) & 0xff];
        }
        std::copy(L, L + 8, state);
    }

    // apply the Miyaguchi-Preneel compression function:
    for (int i = 0; i < 8; ++i) {
        hash_state[i] ^= state[i] ^ block[i];
    }
}

inline void whirlpool_impl::store_bit_count(void* dst) const noexcept
{
    bit_count.store_as_big_endian(dst, 4);
}
